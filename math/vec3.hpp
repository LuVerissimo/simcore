...
#define VEC3_HOST_DEVICE __host__ __device__
#else
#define VEC3_HOST_DEVICE
#endif

// 1. Generic absolute value helper
template <typename T>
[[nodiscard]] VEC3_HOST_DEVICE constexpr T cabs(T x) {
    return x < T(0) ? -x : x;
}
...
struct basic_vec3 {
    ...
    VEC3_HOST_DEVICE constexpr basic_vec3& operator+=(basic_vec3 v) {
        x += v.x; y += v.y; z += v.z;
        return *this;
    }
    VEC3_HOST_DEVICE constexpr basic_vec3& operator-=(basic_vec3 v) {
    ...
    VEC3_HOST_DEVICE constexpr basic_vec3& operator*=(T s) {
        x *= s; y *= s; z *= s;
        return *this;
    }
    VEC3_HOST_DEVICE constexpr basic_vec3& operator/=(T s) {
...
template <typename T>
VEC3_HOST_DEVICE constexpr basic_vec3<T> operator+(basic_vec3<T> a, basic_vec3<T> b) {
...
template <typename T>
VEC3_HOST_DEVICE constexpr basic_vec3<T> operator-(basic_vec3<T> a) {
    return {-a.x, -a.y, -a.z};
}

template <typename T>
VEC3_HOST_DEVICE constexpr basic_vec3<T> operator*(basic_vec3<T> v, T s) {
    return {v.x * s, v.y * s, v.z * s};
}

template <typename T>
VEC3_HOST_DEVICE constexpr basic_vec3<T> operator*(T s, basic_vec3<T> v) {
    return v * s;
}
